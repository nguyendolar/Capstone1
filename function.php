...
function loginFromSocialCallBack($socialUser) {
    ...
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `patient` (`username`,`password`,`patientLastName`,`patientEmail`) VALUES ('" . $socialUser['email'] . "', '" . md5($socialUser['email']) . "', '" . $socialUser['name'] . "', '" . $socialUser['email'] . "')");
        if (!$result) {
